 ...
 * were used when developing the ins and outs of this program. One extra component of this program is using the personally-designed HangmanCanvas to
 * update the hanging man after each turn as opposed to using the display text files. Another extra component is adding in sounds that play after each
 * turn depending on whether the turn was successful or not and sounds that play at the conclusion of each game.
 */

import acm.program.*;
import acm.util.*;
import java.applet.AudioClip; // for Audio
import java.io.*; // for File
import java.util.*; // for Scanner

public class HangmanExtra extends ConsoleProgram {
	private HangmanLexicon lexicon;
	private HangmanCanvas graphics;
	private static final int WRONGGUESSES = 8;
	
	public void init() {
		graphics = new HangmanCanvas();
		add(graphics);
	}
	...
	/*
	 * This displays the welcome message that greets the user at the beginning
	...
	public void play() {
		...
		while (true) {
			...
			while (guessesLeft > 0) { // Displays the gallows according to
				graphics.drawMan(guessesLeft);							// how many guesses are left.
				println("Secret word: " + newHint);
				println("Your guesses so far: " + lettersGuessed);
				println("Guesses left: " + guessesLeft);
				char letter = getGuess(); // Calls what the user has guessed on
											// that turn.
	...
	public String frenchLexicon() {
		ArrayList <String> french = new ArrayList <String> ();
		Scanner newLexicon;
		try {
			newLexicon = new Scanner(new File("res/lexicon-italian.txt"));
			while(newLexicon.hasNextLine()) {	// Each word is added to the array.
				String frenchLexicon = newLexicon.nextLine();
				french.add(frenchLexicon);
			}
		} catch (FileNotFoundException fnfe){
			throw new ErrorException(fnfe);
		}
		RandomGenerator randy = RandomGenerator.getInstance();
		int randomNumber = randy.nextInt(1, french.size());
		return french.get(randomNumber);
	}
	...
	AudioClip applause = MediaTools.loadAudioClip("res/applause_y.wav");
	
	AudioClip boo = MediaTools.loadAudioClip("res/boo.wav");
	
	AudioClip cheering = MediaTools.loadAudioClip("res/cheering.wav");
	
	AudioClip noNoNo = MediaTools.loadAudioClip("res/noNoNo.wav");
	
	AudioClip ohNo = MediaTools.loadAudioClip("res/Oh No.wav");
}