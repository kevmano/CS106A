...
public class Targets extends GraphicsProgram {
	...
	public void target(int size, int startx, int starty, int numCircles) {	//Parameterizes the targets with the parameters being the size, starting x and
		...
		for (int i = 0; i <= numCircles - 1; i++) {
			int x = startx + (size / (2 * numCircles)) * i;					//Finds the x-value of the top-left corner of each successive circle.
			int y = starty + (size / (2 * numCircles)) * i;					//Finds the y-value of the top-left corner of each successive circle.
			int dimension = size - (size / numCircles) * i;					//Finds the size that each successive circle will be.
			if (i % 2 == 0) {												//If the circle number (counting from the outside in) is even, a red circle will
